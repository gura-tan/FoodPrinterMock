 ...
 * デザート・軽食)は小カテゴリが未入力("…etc")。そのため本データでもこの2つのみ
 * 小カテゴリを埋め、残りは空配列(0件)のプレースホルダーとしている(UI側は
 * 「(準備中)」と表示してクラッシュしないようにしてある)。
 * また「メニュー」階層(小カテゴリのさらに先)もNotionに具体データが無いため、
 * デモ用に小カテゴリと同名のメニュー項目を1つだけ仮で持たせ、味調整用の
 * パラメータも仮の値(焼き加減/塩味/量)を設定している。実データが決まり次第
 * menu_data.c を編集するだけでよい。
 */

typedef struct {
    const char *name;          // パラメータ名 (例: "焼き加減")
